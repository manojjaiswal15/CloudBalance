      ...
      <table className="w-full">
        <thead className="bg-sky-200">
          <tr className="border-e">
            ...
            {
              user.role === 'admin' ? <th className="border-e border-gray-200 p-2">Action</th> : ''
            }
          </tr>
        </thead>
        ...
        <tbody>
          {userTableDemoDetails.map((item) => (
            <tr className={`border-e text-center ${(item.id % 2 == 0) ? 'bg-gray-200' : ''}`} key={item.id}>
              ...
              {
                user.role == "admin" ? <td onClick={() => { navigate('/dashboard/users/edit') }} className="border-e border-gray-200 p-2 cursor-pointer text-blue-600"><EditIcon /></td> : ''
              }
            </tr>
          ))}
        </tbody>
